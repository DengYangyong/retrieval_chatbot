...
def evaluate(config, model, data_iter, test=False):
    ...
    return accuracy, loss_total / len(data_iter)
...
def train_model(config, model, train_iter, valid_iter, test_iter):
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, labels) in enumerate(train_iter):
            ...
            """ 训练时，计算损失带权重 """
            loss = F.cross_entropy(outputs, labels, weight=config.class_weights)
            loss.backward()
                        
            optimizer.step()
            
            """ 每训练40个batch就验证一次，如果有提升，就保存并测试一次 """
            if total_batch % 40 == 0:
                
                valid_acc, valid_loss = evaluate(config, model, valid_iter, test=False)
                
                """ 以 accuracy 作为early stop的监控指标 """
                ...
                msg = 'Iter: {} | Train Loss: {:.4f} | Val Loss: {:.4f} | Val accuracy: {:4f} | Time: {} | {}'
                print(msg.format(total_batch, loss.item(), valid_loss, valid_acc, time_dif, improve))
