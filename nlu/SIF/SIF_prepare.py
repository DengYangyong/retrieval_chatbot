...
def load_corpus():
    ...
    """ 进行分词 """
    business_df["question_clean"] = business_df["question"].apply(clean_text)
    chatting_df["question_clean"] = chatting_df["question"].apply(clean_text)
    business_df["answer_clean"] = business_df["answer"].apply(clean_text)
    chatting_df["answer_clean"] = chatting_df["answer"].apply(clean_text)    
    
    business_df.dropna(inplace=True)
    chatting_df.dropna(inplace=True) 
    
    business_df["question_seg"] = business_df["question_clean"].apply(transfer_char)
    chatting_df["question_seg"] = chatting_df["question_clean"].apply(transfer_char)
    business_df["answer_seg"] = business_df["answer_clean"].apply(transfer_char)
    chatting_df["answer_seg"] = chatting_df["answer_clean"].apply(transfer_char)        
